...
A clean review that still consulted the knowledge base does request its findings response, because that response also carries the [source telemetry](/key-features/code-review-sources) used to render the "Sources consulted" panel. The backend returns the telemetry without opening the (empty) findings artifact, so a clean review still avoids the artifact read.
...
The review record's `SourceTelemetryPayload` column holds the compact [source telemetry](/key-features/code-review-sources) snapshot — which knowledge-base documents and snippets the reviewers consulted — captured for both completed and failed runs. It is display and durable-capture data, not the finding bodies.
