...
The embeddings API key ships as a Secret Manager secret (`AppSettings__Llm__Embeddings__ApiKey`), mounted as an environment variable on both the web service and worker pool — see [Google Cloud Configuration](/configuration/gcp-runtime#secrets-required).
