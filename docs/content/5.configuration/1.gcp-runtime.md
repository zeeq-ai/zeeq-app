...
# System-admin allow-list entry in provider:subject format (run after you have first users in database)
...
# OTEL API key (e.g. Datadog, Honeycomb) injected as header
...
cloud-sql-proxy \
  --port 55432 \
  zeeq-mcp-prod:us-central1:zeeq-pg-prod
```
...
This bucket is a **single shared namespace across every organization's
private-source clones** — paths are scoped by org+library only at the
application layer (`LocalTempWorkspaceProvider.ResolveWorkspacePath`), not by
GCS IAM, which has no native per-prefix authorization.
::

`build/ship-worker.sh` mounts this bucket at `/mnt/ingest` and points
...
regardless of storage medium (`LocalIngestWorkspace`'s local-temp semantics
are unchanged) — a repeat sync re-clones rather than reuses a prior pull.
::
...
`GCP_PROJECT_ID` defaults to `zeeq-mcp-prod` in `build/ship.sh` and `build/ship-worker.sh` when it is not set by the caller. Set `GCP_PROJECT_ID` explicitly before running `build/send.sh` to deploy the same image to another project.
