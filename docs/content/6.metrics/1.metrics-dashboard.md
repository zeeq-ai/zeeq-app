...
The home page is a tabbed dashboard of organization metrics rendered with Apache ECharts. Every panel is a single-organization, window-scoped query served by cached read endpoints; the dashboard never reads raw rows. See [Metrics Pipeline](/metrics/metrics-pipeline) for how the underlying data is captured and stored.
