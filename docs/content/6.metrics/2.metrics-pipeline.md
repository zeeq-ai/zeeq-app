...
The dashboard ([Metrics Dashboard](/metrics/metrics-dashboard)) is backed by a durable capture pipeline: in-process `System.Diagnostics.Metrics` instruments are observed, batched, published as a message, and written to a partitioned Postgres table with automatic retention.
