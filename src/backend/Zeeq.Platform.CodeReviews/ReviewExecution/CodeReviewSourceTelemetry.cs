...
///   "tok": { "in": 121518, "cin": 93440, "out": 541, "tot": 122059 },
///   "docs": [
///     {
...
/// <c>tools</c>=toolUsage, <c>miss</c>=missedQueries, <c>tok</c>=tokenUsage.
/// Token usage: <c>in</c>=inputTokens, <c>cin</c>=cachedInputTokens, <c>out</c>=outputTokens,
/// <c>tot</c>=totalTokens. Document/snippet: <c>id</c>=documentId,
...
/// <param name="TokenUsage">Provider-reported review token usage, null when unavailable.</param>
public sealed record CodeReviewSourceTelemetry(
    ...
    [property: JsonPropertyName("tok")] CodeReviewTokenUsage? TokenUsage = null
)
{
    ...
    public bool IsEmpty =>
        Documents.Count == 0
        && ToolUsage.Count == 0
        && MissedQueries.Count == 0
        && TokenUsage is null;
}

/// <summary>Provider-reported token usage for the review run.</summary>
/// <param name="InputTokens">Input tokens, including cached tokens when the provider reports totals.</param>
/// <param name="CachedInputTokens">Cached input tokens, when reported by the provider.</param>
/// <param name="OutputTokens">Output tokens generated by reviewers.</param>
/// <param name="TotalTokens">Total provider-reported tokens.</param>
public sealed record CodeReviewTokenUsage(
    ...
    [property: JsonPropertyName("tot")] long? TotalTokens
);
