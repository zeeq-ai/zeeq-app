...
) : IToolTelemetrySink
{
    ...
    /// <summary>Records provider-reported token usage for one reviewer.</summary>
    /// <param name="inputTokens">Input tokens, including cached tokens when the provider reports totals.</param>
    /// <param name="cachedInputTokens">Cached input tokens reported by the provider.</param>
    /// <param name="outputTokens">Output tokens generated by the reviewer.</param>
    ...
    public CodeReviewSourceTelemetry Snapshot()
    {
        ...
        var tokenUsage = BuildTokenUsage(_tokenUsages.ToArray());
    ...
    private static CodeReviewTokenUsage? BuildTokenUsage(IReadOnlyList<RawTokenUsage> usages)
    {
        if (usages.Count == 0)
        {
            return null;
        ...
        // NOTE: Per-field nullability is intentional because providers do not all report the
        // same usage counters. The UI renders missing fields as "--" while preserving any other
        // token counts that were reported.
        return new(
            InputTokens: SumNullable(usages.Select(usage => usage.InputTokens)),
            CachedInputTokens: SumNullable(usages.Select(usage => usage.CachedInputTokens)),
            OutputTokens: SumNullable(usages.Select(usage => usage.OutputTokens)),
            TotalTokens: SumNullable(usages.Select(usage => usage.TotalTokens))
        );
    }
    ...
    /// <summary>One reviewer run's token usage, preserving null when a provider omitted a field.</summary>
    private sealed record RawTokenUsage(
        long? InputTokens,
        long? CachedInputTokens,
        long? OutputTokens,
        long? TotalTokens
